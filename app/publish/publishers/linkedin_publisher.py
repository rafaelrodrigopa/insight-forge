...
class LinkedInFormatter:
    ...
    def format_for_linkedin(text: str) -> str:
        ...
        # 10. Sanitiza 'M&A' / '&' soltos e converte qualquer texto entre parênteses para o formato '- texto'
        # Previne que o parser RestLi 2.0.0 da API do LinkedIn confunda parênteses brutos (...) com anotações URN e corte o texto
        text = re.sub(r"\bM&A\b", "M e A", text)
        text = re.sub(r"&(?![a-zA-Z0-9#]+;)", "e", text)
        text = re.sub(r"\s*\(([^)]+)\)", r" - \1", text)

        # 10b. Remove emojis duplicados no início de linhas de marcadores (ex: '💡 🚀' -> '🚀')
        text = re.sub(
            r"^([\u2600-\u27bf\U0001f300-\U0001f9ff\u200d]+)\s+([\u2600-\u27bf\U0001f300-\U0001f9ff\u200d]+)\s+",
            r"\2 ",
            text,
            flags=re.MULTILINE,
        )

        # 11. Preserva a frase de CTA dinâmica escolhida pela LLM se contiver 'primeiro comentário', ou insere fallback
        default_cta = "🔗 Confira mais sobre este e outros projetos no meu site, link no primeiro comentário"
        has_cta = re.search(r"[^\n]*(?:primeiro\s+coment[áa]rio|1º\s+coment[áa]rio)[^\n]*", text, re.IGNORECASE)
        if not has_cta:
            hashtag_match = re.search(r"\n(#[A-Za-z0-9_]+\s*)+$", text)
            if hashtag_match:
                start_idx = hashtag_match.start()
                text = text[:start_idx].rstrip() + f"\n\n{default_cta}\n\n" + text[start_idx:].strip()
            else:
                text = text.rstrip() + f"\n\n{default_cta}"
        ...
        try:
            ...
            if image_path and os.path.exists(image_path):
                try:
                    ...
                    print(f"[LinkedInPublisherAdapter] Upload de imagem falhou ({img_err}). Tentando publicação de texto simples...")
